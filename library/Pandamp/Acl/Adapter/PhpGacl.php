...
class Pandamp_Acl_Adapter_PhpGacl
{
	private $_aclEngine;
	private $_acl;
	
	//aParams (host, username, password, dbname)
	function __construct($dbAdapter, $aParams)
	{
		$host = $aParams['host'];
		$username = $aParams['username'];
		$password = $aParams['password'];
		$dbname = $aParams['dbname'];
		...
		$this->_aclEngine = new Pandamp_Acl_Vendor_PhpGaclApi($gacl_options);
		$this->_acl = new Pandamp_Acl_Vendor_PhpGacl($gacl_options);
	}
	function getUsers()
	{
...
//		return $this->_aclEngine->del_group_object($aroGroupId, 'user', $username, $group_type='ARO');
	...
	function getGroupIds($name)
	{
		return $this->_aclEngine->get_group_id($name);
	}
	function getGroupData($groupId)
	{
		return $this->_aclEngine->get_group_data($groupId);
	}
	function getGroups()
	{
		return $this->_aclEngine->getGroups($group_type='ARO');
	}
	function optionsAroGroups()
	{
		return $this->_aclEngine->format_groups($this->_aclEngine->sort_groups('ARO'));
	}
