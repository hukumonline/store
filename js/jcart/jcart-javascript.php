...
function setupSubmitShop(theFrm)
{
	//alert('hi');
	// GET INPUT VALUES FOR USE IN AJAX POST
	var itemId = theFrm.find('input[name=<?php echo $jcart['item_id']?>]').val();
	...
	// SEND ITEM INFO VIA POST TO INTERMEDIATE SCRIPT WHICH CALLS jcart.php AND RETURNS UPDATED CART HTML
	$.post('<?php echo $jcart['path']?>jcart-relay-shop.php', { <?php echo $jcart['item_id']?>: itemId, <?php echo $jcart['item_price']?>: itemPrice, <?php echo $jcart['item_name']?>: itemName, <?php echo $jcart['item_qty']?>: itemQty, <?php echo $jcart['item_add']?> : itemAdd }, function(data) {
...
$(function(){
	...
	$('form.jcartshop').submit(function(){

			setupSubmitShop($(this));
	...
	// WHEN A REMOVE LINK IS CLICKED
	$('#jcartshop a').live('click', function(){
		...
		// SEND ITEM ID VIA POST TO INTERMEDIATE SCRIPT WHICH CALLS jcart.php AND RETURNS UPDATED CART HTML
		$.get('<?php echo $jcart['path']?>jcart-relay-shop.php', { jcart_remove: removeId },
			function(data) {
	...
	// WHEN AN ITEM QTY CHANGES
	$('#jcartshop input').live('change', function(){
		...
		// SEND ITEM INFO VIA POST TO INTERMEDIATE SCRIPT WHICH CALLS jcart.php AND RETURNS UPDATED CART HTML
		$.post('<?php echo $jcart['path']?>jcart-relay-shop.php', { item_id: updateId, item_qty: updateQty, jcart_update_item: '<?php echo $jcart['text']['update_button'];?>' }, function(data) {
			// REPLACE EXISTING CART HTML WITH UPDATED CART HTML
			$('#jcartshop').html(data);
			$('.jcart-hide').remove();
